...
open class IODocument : NSDocument {
    
    // MARK: - SUBTYPES
    
    public typealias DocumentWindowMakerOperation = (IODocument) -> NSWindowController
    ...
    public convenience init(withContent content: AEXMLDocument,
        ...
        let propertyList = PropertyList(withFilename: KCore.PList.Info.rawValue)
        
        self.fileURL = url
        self.content = content
        
        guard let ext = propertyList[IODocument.ExtensionPLKey] as? String else {
            return
        }
